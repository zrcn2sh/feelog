...
Google 로그인 시 `ApiException: 10` (DEVELOPER_ERROR)이 나오면 **패키지 com.idosquare.feelog** 에 대한 **SHA-1**이 Firebase에 등록되지 않은 경우입니다.
...
2. **프로젝트 설정** (톱니바퀴) → **내 앱**  
3. **Android** 앱 중 **패키지 이름이 com.idosquare.feelog 인 앱** 선택  
   - 없으면 **앱 추가** → **Android** → 패키지 이름 `com.idosquare.feelog` 입력 후 앱 등록  
...
**주의:** `com.example.feelog_app` 앱이 아니라 **com.idosquare.feelog** 앱에 SHA-1을 추가해야 합니다.
